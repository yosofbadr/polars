...
pub fn tree_traversal<Key, Storage, Edge>(
    root_key: Key,
    storage: &mut Storage,
    visit_stack: &mut Vec<Key>,
    edges: &mut Vec<Edge>,
    visitor: &mut dyn NodeVisitor<Key = Key, Storage = Storage, Edge = Edge>,
) -> PolarsResult<Edge>
where
    Key: Clone,
    Storage: GetNodeInputs<Key>,
{
    ...
    let _: ControlFlow<_, _> = tree_traversal_impl::<Key, Storage, Edge>(
        root_key,
        root_edge_idx,
        storage,
        visit_stack,
...
#[recursive::recursive]
pub fn tree_traversal_impl<Key, Storage, Edge>(
    current_key: Key,
    ...
    Storage: GetNodeInputs<Key>,
{
    ...
    match visitor.pre_visit(
        ...
        CF::Continue(SubtreeVisit::Visit) => {
            for i in 0..num_inputs {
                match tree_traversal_impl(
